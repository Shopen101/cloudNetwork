...
export default function SignIn() {
    ...
    const authHandler = async () => {
        try {
            localStorage.setItem('selectItem', 'новая схема 1')
            setSnackErrTxt('')
            await api.post('auth/login', { ...form })
                .then((response) => login(response.data.token, response.data.userId, response.data.name, response.data.surname))
            history.push('/Chart')
        } catch (err) {
